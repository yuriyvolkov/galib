...
GAGenome &GARankSelector::select() const
{
    ...
    GAPopulation::SortBasis basis = (which == SCALED ? GAPopulation::SCALED : GAPopulation::RAW);
    pop->sort(gaFalse, basis);
    if (which == SCALED) {
        for (i = 1; i < pop->size() && pop->best(i, basis).fitness() == pop->best(0, basis).fitness(); i++)
            count++;
    } else {
        for (i = 1; i < pop->size() && pop->best(i, basis).score() == pop->best(0, basis).score(); i++)
            count++;
    }
...
GAGenome &GAUniformSelector::select() const { return pop->individual(GARandomInt(0, pop->size() - 1)); }
#endif
    ...
    if (which == GASelectionScheme::RAW) {
        ...
        if (pop->fitmax() == pop->fitmin()) {
            ...
            if (pop->order() == GAPopulation::HIGH_IS_BEST) {
                ...
                for (i = 1; i < n; i++)
                    psum[i] = -pop->individual(i, GAPopulation::SCALED).fitness() + pop->fitmax() + pop->fitmin() +
                              psum[i - 1];
                for (i = 0; i < n; i++)
                    psum[i] /= psum[n - 1];
            }
...
GAGenome &GATournamentSelector::select() const
{
    ...
    if (pop->order() == GAPopulation::LOW_IS_BEST) {
        if (pop->individual(lower, basis).score() < pop->individual(picked, basis).score())
            picked = lower;
    } else {
        if (pop->individual(lower, basis).score() > pop->individual(picked, basis).score())
            picked = lower;
    ...
    if (which == GASelectionScheme::RAW) {
        if (pop->ave() == 0 || pop->max() == pop->min()) {
            ...
            for (i = 0; i < pop->size(); i++) {
                if (pop->order() == GAPopulation::HIGH_IS_BEST)
                    expected = pop->individual(i, GAPopulation::RAW).score() / pop->ave();
                else
                    expected = (-pop->individual(i, GAPopulation::RAW).score() + pop->max() + pop->min()) / pop->ave();
                ne = (int)expected;
            ...
            GAErr(GA_LOC, className(), "update", "objective scores are not strictly negative or strictly positive",
                  "this selection method cannot be used with these scores");
        }
    } else {
        if (pop->fitave() == 0 || pop->fitmax() == pop->fitmin()) {
            ...
            for (i = 0; i < pop->size(); i++) {
                if (pop->order() == GAPopulation::HIGH_IS_BEST)
                    expected = pop->individual(i, GAPopulation::SCALED).fitness() / pop->fitave();
                else
                    expected = (-pop->individual(i, GAPopulation::SCALED).fitness() + pop->fitmax() + pop->fitmin()) /
                               pop->fitave();
                ne = (int)expected;
                fraction[i] = expected - ne;
                while (ne > 0 && k < (int)n) {
            ...
            GAErr(GA_LOC, className(), "update", "fitness scores are not strictly negative or strictly positive",
                  "this selection method cannot be used with these scores");
        }
    }
}
