...
GASteadyStateGA::GASteadyStateGA(const GASteadyStateGA& ga) : GAGeneticAlgorithm(ga)
{
...
int GASteadyStateGA::setptr(const char* name, const void* value)
{
    ...
    if (strcmp(name, gaNpReplacement) == 0 || strcmp(name, gaSNpReplacement) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((float*)value) << "'\n";
#endif
        pReplacement(*((float*)value));
        status = 0;
    } else if (strcmp(name, gaNnReplacement) == 0 || strcmp(name, gaSNnReplacement) == 0) {
#ifdef GA_DEBUG
...
void GASteadyStateGA::step()
{
    ...
    if (tmpPop->size() % 2 != 0) { // do the remaining population member
        ...
        c1 = 0;
        if (GAFlipCoin(pCrossover())) {
            stats.numcro += (*scross)(*mom, *dad, &tmpPop->individual(i), (GAGenome*)0);
            c1 = 1;
        } else {
            if (GARandomBit())
                tmpPop->individual(i).copy(*mom);
