...
class GA3DArrayGenome : public GAArray<T>, public GAGenome
{
  ...
  public:
    GA3DArrayGenome(unsigned int x, unsigned int y, unsigned int z, GAGenome::Evaluator f = (GAGenome::Evaluator)0,
                    void* u = (void*)0);
    ...
    const T& gene(unsigned int x, unsigned int y, unsigned int z) const { return this->a[z * ny * nx + y * nx + x]; }
    ...
    {
        return (resizeBehaviour(WIDTH, lowerX, upperX) * resizeBehaviour(HEIGHT, lowerY, upperY) *
                resizeBehaviour(DEPTH, lowerZ, upperZ));
    }
    void copy(const GA3DArrayGenome&, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
              unsigned int, unsigned int, unsigned int, unsigned int);
    void swap(unsigned int a, unsigned int b, unsigned int c, unsigned int d, unsigned int e, unsigned int f)
    {
...
class GA3DArrayAlleleGenome : public GA3DArrayGenome<T>
{
  ...
  public:
    GA3DArrayAlleleGenome(unsigned int x, unsigned int y, unsigned int z, const GAAlleleSet<T>& a,
                          GAGenome::Evaluator f = (GAGenome::Evaluator)0, void* u = (void*)0);
    GA3DArrayAlleleGenome(unsigned int x, unsigned int y, unsigned int z, const GAAlleleSetArray<T>& a,
                          GAGenome::Evaluator f = (GAGenome::Evaluator)0, void* u = (void*)0);
...
---------------------------------------------------------------------------- */
template <class T>
const char *GA3DArrayGenome<T>::className() const
{
    return "GA3DArrayGenome";
}
template <class T>
int GA3DArrayGenome<T>::classID() const
{
    return GAID::ArrayGenome3D;
}

template <class T>
GA3DArrayGenome<T>::GA3DArrayGenome(unsigned int w, unsigned int h, unsigned int d, GAGenome::Evaluator f, void *u)
        : GAArray<T>(w * h * d), GAGenome(DEFAULT_3DARRAY_INITIALIZER, DEFAULT_3DARRAY_MUTATOR, DEFAULT_3DARRAY_COMPARATOR)
{
    evaluator(f);
    userData(u);
...
template <class T>
void GA3DArrayGenome<T>::copy(const GAGenome &orig)
{
    if (&orig == this)
        return;
...
template <class T>
int GA3DArrayGenome<T>::resize(int w, int h, int d)
{
...
#ifdef GALIB_USE_STREAMS
template <class T>
int GA3DArrayGenome<T>::read(STD_ISTREAM &)
{
...
template <class T>
int GA3DArrayGenome<T>::write(STD_OSTREAM &os) const
{
    for (unsigned int k = 0; k < nz; k++) {
        for (unsigned int j = 0; j < ny; j++) {
            for (unsigned int i = 0; i < nx; i++) {
...
template <class T>
int GA3DArrayGenome<T>::resizeBehaviour(GAGenome::Dimension which, unsigned int lower, unsigned int upper)
{
    if (upper < lower) {
...
template <class T>
void GA3DArrayGenome<T>::copy(const GA3DArrayGenome<T> &orig, unsigned int r, unsigned int s, unsigned int t,
                              unsigned int x, unsigned int y, unsigned int z, unsigned int w, unsigned int h,
                              unsigned int d)
{
    if (w == 0 || x >= orig.nx || r >= nx || h == 0 || y >= orig.ny || s >= ny || d == 0 || z >= orig.nz || t >= nz)
        return;
    if (x + w > orig.nx)
        w = orig.nx - x;
    if (y + h > orig.ny)
    ...
    for (unsigned int k = 0; k < d; k++)
        for (unsigned int j = 0; j < h; j++)
            GAArray<T>::copy(orig, (t + k) * ny * nx + (s + j) * nx + r,
...
template <class T>
int GA3DArrayGenome<T>::equal(const GAGenome &c) const
{
    if (this == &c)
        return 1;
...
---------------------------------------------------------------------------- */
template <class T>
const char *GA3DArrayAlleleGenome<T>::className() const
{
    return "GA3DArrayAlleleGenome";
}
template <class T>
int GA3DArrayAlleleGenome<T>::classID() const
{
    return GAID::ArrayAlleleGenome3D;
}

template <class T>
GA3DArrayAlleleGenome<T>::GA3DArrayAlleleGenome(unsigned int w, unsigned int h, unsigned int d, const GAAlleleSet<T> &s,
        ...
        : GA3DArrayGenome<T>(w, h, d, f, u)
{
    ...
    GAGenome::comparator(GA3DArrayAlleleGenome<T>::DEFAULT_3DARRAY_ALLELE_COMPARATOR);
    GAGenome::crossover(GA3DArrayAlleleGenome<T>::DEFAULT_3DARRAY_ALLELE_CROSSOVER);
}

template <class T>
GA3DArrayAlleleGenome<T>::GA3DArrayAlleleGenome(unsigned int w, unsigned int h, unsigned int d,
                                                const GAAlleleSetArray<T> &sa, GAGenome::Evaluator f, void *u)
        : GA3DArrayGenome<T>(w, h, d, f, u)
{
    naset = sa.size();
...
template <class T>
GA3DArrayAlleleGenome<T>::GA3DArrayAlleleGenome(const GA3DArrayAlleleGenome<T> &orig)
...
template <class T>
void GA3DArrayAlleleGenome<T>::copy(const GAGenome &orig)
{
    if (&orig == this)
        return;
    const GA3DArrayAlleleGenome<T> *c = DYN_CAST(const GA3DArrayAlleleGenome<T> *, &orig);
    if (c) {
...
template <class T>
int GA3DArrayAlleleGenome<T>::resize(int w, int h, int d)
{
...
#ifdef GALIB_USE_STREAMS
template <class T>
int GA3DArrayAlleleGenome<T>::read(STD_ISTREAM &is)
{
    return GA3DArrayGenome<T>::read(is);
}

template <class T>
int GA3DArrayAlleleGenome<T>::write(STD_OSTREAM &os) const
{
...
template <class T>
int GA3DArrayAlleleGenome<T>::equal(const GAGenome &c) const
{
...
{
    GA3DArrayAlleleGenome<ARRAY_TYPE> &child = DYN_CAST(GA3DArrayAlleleGenome<ARRAY_TYPE> &, c);
...
template <class ARRAY_TYPE>
float GA3DArrayGenome<ARRAY_TYPE>::ElementComparator(const GAGenome &a, const GAGenome &b)
{
    const GA3DArrayGenome<ARRAY_TYPE> &sis = DYN_CAST(const GA3DArrayGenome<ARRAY_TYPE> &, a);
    const GA3DArrayGenome<ARRAY_TYPE> &bro = DYN_CAST(const GA3DArrayGenome<ARRAY_TYPE> &, b);
...
template <class T>
int GA3DArrayGenome<T>::UniformCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    ...
    if (c1 && c2) {
        ...
        if (sis.width() == bro.width() && sis.height() == bro.height() && sis.depth() == bro.depth() &&
            mom.width() == dad.width() && mom.height() == dad.height() && mom.depth() == dad.depth() &&
            sis.width() == mom.width() && sis.height() == mom.height() && sis.depth() == mom.depth()) {
            ...
            for (i = startx - 1; i >= 0; i--)
                for (j = starty - 1; j >= 0; j--)
                    for (k = startz - 1; k >= 0; k--)
                        sis.gene(i, j, k,
                                 (mask[i * starty * startz + j * startz + k] ? mom.gene(i, j, k) : dad.gene(i, j, k)));
            ...
            for (i = startx - 1; i >= 0; i--)
                for (j = starty - 1; j >= 0; j--)
                    for (k = startz - 1; k >= 0; k--)
                        bro.gene(i, j, k,
                                 (mask[i * starty * startz + j * startz + k] ? dad.gene(i, j, k) : mom.gene(i, j, k)));
        }
        ...
        if (mom.width() == dad.width() && mom.height() == dad.height() && mom.depth() == dad.depth() &&
            sis.width() == mom.width() && sis.height() == mom.height() && sis.depth() == mom.depth()) {
            for (i = sis.width() - 1; i >= 0; i--)
                for (j = sis.height() - 1; j >= 0; j--)
                    for (k = sis.depth() - 1; k >= 0; k--)
...
// It might break!
template <class T>
int GA3DArrayGenome<T>::EvenOddCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GA3DArrayGenome<T> &mom = DYN_CAST(const GA3DArrayGenome<T> &, p1);
    const GA3DArrayGenome<T> &dad = DYN_CAST(const GA3DArrayGenome<T> &, p2);
    ...
    if (c1 && c2) {
        ...
        if (sis.width() == bro.width() && sis.height() == bro.height() && sis.depth() == bro.depth() &&
            ...
            for (i = startx - 1; i >= 0; i--)
                for (j = starty - 1; j >= 0; j--)
                    for (k = startz - 1; k >= 0; k--)
                        sis.gene(i, j, k, (((i * starty * startz + j * startz + k) % 2 == 0) ? mom.gene(i, j, k)
                                                                                             : dad.gene(i, j, k)));
            startx = (bro.width() < minx) ? bro.width() : minx;
            starty = (bro.height() < miny) ? bro.height() : miny;
            startz = (bro.depth() < minz) ? bro.depth() : minz;
            for (i = startx - 1; i >= 0; i--)
                for (j = starty - 1; j >= 0; j--)
                    for (k = startz - 1; k >= 0; k--)
                        bro.gene(i, j, k, (((i * starty * startz + j * startz + k) % 2 == 0) ? dad.gene(i, j, k)
                                                                                             : mom.gene(i, j, k)));
        }
        ...
        if (mom.width() == dad.width() && mom.height() == dad.height() && mom.depth() == dad.depth() &&
            ...
            for (i = minx - 1; i >= 0; i--)
                for (j = miny - 1; j >= 0; j--)
                    for (k = minz - 1; k >= 0; k--)
                        sis.gene(i, j,
                                 (((i * miny * minz + j * minz + k) % 2 == 0) ? mom.gene(i, j, k) : dad.gene(i, j, k)));
        }
...
template <class T>
int GA3DArrayGenome<T>::OnePointCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GA3DArrayGenome<T> &mom = DYN_CAST(const GA3DArrayGenome<T> &, p1);
    const GA3DArrayGenome<T> &dad = DYN_CAST(const GA3DArrayGenome<T> &, p2);
    ...
    if (c1 && c2) {
        ...
        if (sis.resizeBehaviour(GAGenome::WIDTH) == GAGenome::FIXED_SIZE &&
            ...
            if (mom.width() != dad.width() || sis.width() != bro.width() || sis.width() != mom.width()) {
        ...
        if (sis.resizeBehaviour(GAGenome::HEIGHT) == GAGenome::FIXED_SIZE &&
            ...
            if (mom.height() != dad.height() || sis.height() != bro.height() || sis.height() != mom.height()) {
                GAErr(GA_LOC, mom.className(), "one-point cross", gaErrSameLengthReqd);
                return nc;
            }
        ...
        if (sis.resizeBehaviour(GAGenome::DEPTH) == GAGenome::FIXED_SIZE &&
            ...
            if (mom.depth() != dad.depth() || sis.depth() != bro.depth() || sis.depth() != mom.depth()) {
                GAErr(GA_LOC, mom.className(), "one-point cross", gaErrSameLengthReqd);
                return nc;
            }
        ...
        sis.copy(mom, 0, 0, 0, momsitex - sitex, momsitey - sitey, momsitez - sitez, sitex, sitey, sitez);
        sis.copy(dad, sitex, 0, 0, dadsitex, dadsitey - sitey, dadsitez - sitez, lenx, sitey, sitez);
        sis.copy(dad, 0, sitey, 0, dadsitex - sitex, dadsitey, dadsitez - sitez, sitex, leny, sitez);
        sis.copy(mom, sitex, sitey, 0, momsitex, momsitey, momsitez - sitez, lenx, leny, sitez);
        sis.copy(dad, 0, 0, sitez, dadsitex - sitex, dadsitey - sitey, dadsitez, sitex, sitey, lenz);
        sis.copy(mom, sitex, 0, sitez, momsitex, momsitey - sitey, momsitez, lenx, sitey, lenz);
        sis.copy(mom, 0, sitey, sitez, momsitex - sitex, momsitey, momsitez, sitex, leny, lenz);
        sis.copy(dad, sitex, sitey, sitez, dadsitex, dadsitey, dadsitez, lenx, leny, lenz);

        bro.copy(dad, 0, 0, 0, dadsitex - sitex, dadsitey - sitey, dadsitez - sitez, sitex, sitey, sitez);
        bro.copy(mom, sitex, 0, 0, momsitex, momsitey - sitey, momsitez - sitez, lenx, sitey, sitez);
        bro.copy(mom, 0, sitey, 0, momsitex - sitex, momsitey, momsitez - sitez, sitex, leny, sitez);
        bro.copy(dad, sitex, sitey, 0, dadsitex, dadsitey, dadsitez - sitez, lenx, leny, sitez);
        bro.copy(mom, 0, 0, sitez, momsitex - sitex, momsitey - sitey, momsitez, sitex, sitey, lenz);
        bro.copy(dad, sitex, 0, sitez, dadsitex, dadsitey - sitey, dadsitez, lenx, sitey, lenz);
        bro.copy(dad, 0, sitey, sitez, dadsitex - sitex, dadsitey, dadsitez, sitex, leny, lenz);
        bro.copy(mom, sitex, sitey, sitez, momsitex, momsitey, momsitez, lenx, leny, lenz);
        ...
        if (GARandomBit()) {
            ...
            sis.copy(dad, 0, 0, 0, dadsitex - sitex, dadsitey - sitey, dadsitez - sitez, sitex, sitey, sitez);
            sis.copy(mom, sitex, 0, 0, momsitex, momsitey - sitey, momsitez - sitez, lenx, sitey, sitez);
            sis.copy(mom, 0, sitey, 0, momsitex - sitex, momsitey, momsitez - sitez, sitex, leny, sitez);
            sis.copy(dad, sitex, sitey, 0, dadsitex, dadsitey, dadsitez - sitez, lenx, leny, sitez);
            sis.copy(mom, 0, 0, sitez, momsitex - sitex, momsitey - sitey, momsitez, sitex, sitey, lenz);
            sis.copy(dad, sitex, 0, sitez, dadsitex, dadsitey - sitey, dadsitez, lenx, sitey, lenz);
            sis.copy(dad, 0, sitey, sitez, dadsitex - sitex, dadsitey, dadsitez, sitex, leny, lenz);
            sis.copy(mom, sitex, sitey, sitez, momsitex, momsitey, momsitez, lenx, leny, lenz);
        }
