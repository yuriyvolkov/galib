...
GABoolean GAGeneticAlgorithm::TerminateUponPopConvergence(GAGeneticAlgorithm& ga)
{
    ...
    float ratio = ga.statistics().current(GAStatistics::Minimum) / ga.statistics().current(GAStatistics::Maximum);
...
GAParameterList& GAGeneticAlgorithm::registerDefaultParameters(GAParameterList& p)
{
    ...
    p.add(gaNpopulationSize, gaSNpopulationSize, GAParameter::INT, &gaDefPopSize);
    ...
    p.add(gaNrecordDiversity, gaSNrecordDiversity, GAParameter::INT, &gaDefDivFlag);
    p.add(gaNscoreFilename, gaSNscoreFilename, GAParameter::STRING, gaDefScoreFilename);
    p.add(gaNselectScores, gaSNselectScores, GAParameter::INT, &gaDefSelectScores);
...
GAGeneticAlgorithm::GAGeneticAlgorithm(const GAGenome& g) : stats(), params()
{
    ...
    params.add(gaNrecordDiversity, gaSNrecordDiversity, GAParameter::INT, &gaDefDivFlag);
    ...
    params.add(gaNselectScores, gaSNselectScores, GAParameter::INT, &gaDefSelectScores);
...
const GAParameterList& GAGeneticAlgorithm::parameters(const GAParameterList& list)
{
...
const GAParameterList& GAGeneticAlgorithm::parameters(int& argc, char** argv, GABoolean flag)
{
...
const GAParameterList& GAGeneticAlgorithm::parameters(const char* filename, GABoolean flag)
{
...
const GAParameterList& GAGeneticAlgorithm::parameters(STD_ISTREAM& is, GABoolean flag)
{
    params.read(is, flag);
    for (int i = 0; i < params.size(); i++)
        setptr(params[i].fullname(), params[i].value());
...
int GAGeneticAlgorithm::setptr(const char* name, const void* value)
{
    ...
    if (strcmp(name, gaNnBestGenomes) == 0 || strcmp(name, gaSNnBestGenomes) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        nBestGenomes(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNpopulationSize) == 0 || strcmp(name, gaSNpopulationSize) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        ...
        minimaxi(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNnGenerations) == 0 || strcmp(name, gaSNnGenerations) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        nGenerations(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNpConvergence) == 0 || strcmp(name, gaSNpConvergence) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((float*)value) << "'\n";
#endif
        pConvergence(*((float*)value));
        status = 0;
    } else if (strcmp(name, gaNnConvergence) == 0 || strcmp(name, gaSNnConvergence) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        nConvergence(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNpCrossover) == 0 || strcmp(name, gaSNpCrossover) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((float*)value) << "'\n";
#endif
        pCrossover(*((float*)value));
        status = 0;
    } else if (strcmp(name, gaNpMutation) == 0 || strcmp(name, gaSNpMutation) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((float*)value) << "'\n";
#endif
        pMutation(*((float*)value));
        status = 0;
    } else if (strcmp(name, gaNscoreFrequency) == 0 || strcmp(name, gaSNscoreFrequency) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        stats.scoreFrequency(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNflushFrequency) == 0 || strcmp(name, gaSNflushFrequency) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        stats.flushFrequency(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNrecordDiversity) == 0 || strcmp(name, gaSNrecordDiversity) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        stats.recordDiversity(*((int*)value) ? gaTrue : gaFalse);
        status = 0;
    } else if (strcmp(name, gaNselectScores) == 0 || strcmp(name, gaSNselectScores) == 0) {
#ifdef GA_DEBUG
        cerr << "GAGeneticAlgorithm::setptr\n  setting '" << name << "' to '" << *((int*)value) << "'\n";
#endif
        stats.selectScores(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNscoreFilename) == 0 || strcmp(name, gaSNscoreFilename) == 0) {
#ifdef GA_DEBUG
...
int GAGeneticAlgorithm::get(const char* name, void* value) const
{
    ...
    if (strcmp(name, gaNseed) == 0 || strcmp(name, gaSNseed) == 0) {
        *((int*)value) = d_seed;
        status = 0;
    } else if (strcmp(name, gaNnBestGenomes) == 0 || strcmp(name, gaSNnBestGenomes) == 0) {
