...
class GATree : public GATreeBASE
{
  public:
    ...
    T *warp(const GATreeIter<T> &i) { return ((i.tree == this) ? iter.warp(i) : (T *)0); }
...
class GATreeIter : public GATreeIterBASE
{
  public:
    ...
    T *next() { return ((node && node->next) ? &((GANode<T> *)(node = node->next))->contents : (T *)0); }
    T *prev() { return ((node && node->prev) ? &((GANode<T> *)(node = node->prev))->contents : (T *)0); }
    T *parent() { return ((node && node->parent) ? &((GANode<T> *)(node = node->parent))->contents : (T *)0); }
    T *child() { return ((node && node->child) ? &((GANode<T> *)(node = node->child))->contents : (T *)0); }
    T *eldest() { return (node ? &((GANode<T> *)GATreeIterBASE::eldest(node))->contents : (T *)0); }
    T *youngest() { return (node ? &((GANode<T> *)GATreeIterBASE::youngest(node))->contents : (T *)0); }
...
template <class T>
void GATree<T>::copy(const GATree<T> &orig)
{
    _GATreeDestroy(DYN_CAST(GANode<T> *, rt));
...
template <class T>
GATree<T> *GATree<T>::clone(unsigned int i) const
{
    GATree<T> *t = new GATree<T>;
    GANode<T> *node;
    unsigned int w = 0;
...
template <class T>
int GATree<T>::swaptree(GATree<T> *t)
{
    GANodeBASE *tmp;
    if (t->iter.node && iter.node) {
        ...
        if (!rt) {             // should always be true at this point
            ...
            //      tmp->prev = tmp;
            t->iter.node = (GANodeBASE *)0;
            if (insert(DYN_CAST(GANode<T> *, tmp), (GANode<T> *)0, GATreeBASE::ROOT) == GATreeBASE::ERR)
                return GATreeBASE::ERR;
        }
        ...
        if (!t->rt) {       // should always be true
            ...
            //      tmp->prev = tmp;
            iter.node = (GANodeBASE *)0;
            if (t->insert(DYN_CAST(GANode<T> *, tmp), (GANode<T> *)0, GATreeBASE::ROOT) == GATreeBASE::ERR)
                return GATreeBASE::ERR;
        }
    }
...
// swap.
template <class T>
int GATree<T>::swaptree(unsigned int a, unsigned int b)
{
...
template <class T>
int GATree<T>::swap(unsigned int a, unsigned int b)
{
    unsigned int aw = 0, bw = 0;
    GANodeBASE *anode = _GATreeTraverse(a, aw, rt);
    GANodeBASE *bnode = _GATreeTraverse(b, bw, rt);
...
template <class T>
GANode<T> *_GATreeCopy(GANode<T> *node, GANode<T> *parent)
{
    if (!node)
        return (GANode<T> *)0;
...
template <class T>
void _GATreeDestroy(GANode<T> *node)
{
    if (!node)
        return;
