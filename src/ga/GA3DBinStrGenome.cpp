...
---------------------------------------------------------------------------- */
GA3DBinaryStringGenome::GA3DBinaryStringGenome(unsigned int width, unsigned int height, unsigned int depth,
                                               GAGenome::Evaluator f, void *u)
    : GABinaryString(width * height * depth),
      GAGenome(DEFAULT_3DBINSTR_INITIALIZER, DEFAULT_3DBINSTR_MUTATOR, DEFAULT_3DBINSTR_COMPARATOR)
{
    evaluator(f);
    userData(u);
...
GA3DBinaryStringGenome::GA3DBinaryStringGenome(const GA3DBinaryStringGenome &orig)
...
void GA3DBinaryStringGenome::copy(const GAGenome &orig)
{
    if (&orig == this)
        return;
    const GA3DBinaryStringGenome *c = DYN_CAST(const GA3DBinaryStringGenome *, &orig);
    if (c) {
        GAGenome::copy(*c);
...
int GA3DBinaryStringGenome::resizeBehaviour(Dimension which, unsigned int lower, unsigned int upper)
{
    if (upper < lower) {
...
void GA3DBinaryStringGenome::copy(const GA3DBinaryStringGenome &orig, unsigned int r, unsigned int s, unsigned int t,
                                  ...
                                  unsigned int d)
{
    ...
    for (unsigned int k = 0; k < d; k++)
        for (unsigned int j = 0; j < h; j++)
            GABinaryString::copy(orig, (t + k) * ny * nx + (s + j) * nx + r,
                                 (z + k) * orig.ny * orig.nx + (y + j) * orig.nx + x, w);
    _evaluated = gaFalse;
}

void GA3DBinaryStringGenome::set(unsigned int x, unsigned int y, unsigned int z, unsigned int w, unsigned int h,
                                 unsigned int d)
{
    if (x + w > nx)
        w = nx - x;
...
void GA3DBinaryStringGenome::unset(unsigned int x, unsigned int y, unsigned int z, unsigned int w, unsigned int h,
                                   unsigned int d)
{
    if (x + w > nx)
        w = nx - x;
...
void GA3DBinaryStringGenome::randomize(unsigned int x, unsigned int y, unsigned int z, unsigned int w, unsigned int h,
                                       unsigned int d)
{
    if (x + w > nx)
        w = nx - x;
...
void GA3DBinaryStringGenome::move(unsigned int x, unsigned int y, unsigned int z, unsigned int srcx, unsigned int srcy,
                                  unsigned int srcz, unsigned int w, unsigned int h, unsigned int d)
{
    if (srcx + w > nx)
        w = nx - srcx;
    if (x + w > nx)
...
int GA3DBinaryStringGenome::equal(const GA3DBinaryStringGenome &orig, unsigned int x, unsigned int y, unsigned int z,
                                  unsigned int srcx, unsigned int srcy, unsigned int srcz, unsigned int w,
                                  unsigned int h, unsigned int d) const
{
    unsigned int eq = 0;
    for (unsigned int k = 0; k < d; k++)
        for (unsigned int j = 0; j < h; j++)
            eq += GABinaryString::equal(orig, (z + k) * ny * nx + (y + j) * nx + x,
                                        (srcz + k) * ny * nx + (srcy + j) * nx + srcx, w);
    return eq == d * h ? 1 : 0;
}
...
int GA3DBinaryStringGenome::UniformCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GA3DBinaryStringGenome &mom = DYN_CAST(const GA3DBinaryStringGenome &, p1);
    const GA3DBinaryStringGenome &dad = DYN_CAST(const GA3DBinaryStringGenome &, p2);
        ...
        if (sis.width() == bro.width() && sis.height() == bro.height() && sis.depth() == bro.depth() &&
            ...
            for (i = minx - 1; i >= 0; i--)
                for (j = miny - 1; j >= 0; j--)
                    for (k = minz - 1; k >= 0; k--)
                        sis.gene(i, j, k,
                                 (mask[i * miny * minz + j * minz + k] ? mom.gene(i, j, k) : dad.gene(i, j, k)));
            ...
            for (i = minx - 1; i >= 0; i--)
                for (j = miny - 1; j >= 0; j--)
                    for (k = minz - 1; k >= 0; k--)
                        bro.gene(i, j, k,
                                 (mask[i * miny * minz + j * minz + k] ? dad.gene(i, j, k) : mom.gene(i, j, k)));
        }

        nc = 2;
    } else if (c1 || c2) {
        GA3DBinaryStringGenome &sis =
            (c1 ? DYN_CAST(GA3DBinaryStringGenome &, *c1) : DYN_CAST(GA3DBinaryStringGenome &, *c2));
...
// It might break!
int GA3DBinaryStringGenome::EvenOddCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GA3DBinaryStringGenome &mom = DYN_CAST(const GA3DBinaryStringGenome &, p1);
    const GA3DBinaryStringGenome &dad = DYN_CAST(const GA3DBinaryStringGenome &, p2);
        ...
        if (sis.width() == bro.width() && sis.height() == bro.height() && sis.depth() == bro.depth() &&
            ...
            for (i = minx - 1; i >= 0; i--)
                for (j = miny - 1; j >= 0; j--)
                    for (k = minz - 1; k >= 0; k--)
                        bro.gene(i, j, k,
                                 (((i * miny * minz + j * minz + k) % 2 == 0) ? dad.gene(i, j, k) : mom.gene(i, j, k)));
        }
...
int GA3DBinaryStringGenome::OnePointCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GA3DBinaryStringGenome &mom = DYN_CAST(const GA3DBinaryStringGenome &, p1);
    const GA3DBinaryStringGenome &dad = DYN_CAST(const GA3DBinaryStringGenome &, p2);
