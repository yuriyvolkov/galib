...
class GAAlleleSetCore
{
  public:
    ...
    GAAlleleSetCore(const T& lower, const T& upper, const T& increment, GAAllele::BoundType lb = GAAllele::INCLUSIVE,
                    GAAllele::BoundType ub = GAAllele::INCLUSIVE);
...
class GAAlleleSet
{
  public:
    ...
    GAAlleleSet(const T& lower, const T& upper, GAAllele::BoundType lb = GAAllele::INCLUSIVE,
                GAAllele::BoundType ub = GAAllele::INCLUSIVE)
        : core(new GAAlleleSetCore<T>(lower, upper, lb, ub))
    {
    }
    GAAlleleSet(const T& lower, const T& upper, const T& increment, GAAllele::BoundType lb = GAAllele::INCLUSIVE,
                GAAllele::BoundType ub = GAAllele::INCLUSIVE)
...
class GAAlleleSetArray
{
  public:
    ...
    int add(const T& lower, const T& upper, GAAllele::BoundType lb = GAAllele::INCLUSIVE,
            GAAllele::BoundType ub = GAAllele::INCLUSIVE);
    int add(const T& lower, const T& upper, const T& increment, GAAllele::BoundType lb = GAAllele::INCLUSIVE,
            GAAllele::BoundType ub = GAAllele::INCLUSIVE);
...
#ifdef GALIB_USE_STREAMS
template <class T>
STD_OSTREAM& operator<<(STD_OSTREAM& os, const GAAlleleSet<T>& arg)
{
    arg.write(os);
    return os;
}
template <class T>
STD_ISTREAM& operator>>(STD_ISTREAM& is, GAAlleleSet<T>& arg)
{
    arg.read(is);
    return is;
}
...
template <class T>
GAAlleleSetCore<T>::GAAlleleSetCore(const T& lower, const T& upper, GAAllele::BoundType lb, GAAllele::BoundType ub)
...
template <class T>
GAAlleleSetCore<T>::GAAlleleSetCore(const T& lower, const T& upper, const T& increment, GAAllele::BoundType lb,
...
template <class T>
GAAlleleSetCore<T>& GAAlleleSetCore<T>::operator=(const GAAlleleSetCore<T>& orig)
{
    if (this == &orig)
        return *this;
...
template <class T>
void GAAlleleSet<T>::link(GAAlleleSet<T>& set)
{
    if (&set != this) {
        if (core != 0) {
            core->cnt -= 1;
...
template <class T>
int GAAlleleSet<T>::add(const T& alle)
{
    if (core == 0)
...
template <class T>
int GAAlleleSet<T>::remove(const T& allele)
{
    if (core == 0)
        core = new GAAlleleSetCore<T>;
    if (core->type != GAAllele::ENUMERATED)
...
template <class T>
int GAAlleleSet<T>::remove(unsigned int x)
{
...
template <class T>
T GAAlleleSet<T>::allele(unsigned int i) const
{
    if (core->type == GAAllele::ENUMERATED)
...
#ifdef GALIB_USE_STREAMS
template <class T>
int GAAlleleSet<T>::read(STD_ISTREAM&)
{
    GAErr(GA_LOC, "GAAlleleSet", "read", gaErrOpUndef);
    return 1;
}
template <class T>
int GAAlleleSet<T>::write(STD_OSTREAM&) const
{
...
GAAlleleSetArray<T>::GAAlleleSetArray(const GAAlleleSet<T>& s)
        : csz(GA_ALLELE_CHUNK), sz(1), SZ(GA_ALLELE_CHUNK), aset(new GAAlleleSet<T>* [GA_ALLELE_CHUNK])
{
...
GAAlleleSetArray<T>::GAAlleleSetArray(const GAAlleleSetArray<T>& orig)
        : csz(orig.csz), sz(orig.sz), SZ(orig.SZ), aset(new GAAlleleSet<T>* [orig.SZ])
{
    for (unsigned int i = 0; i < sz; i++)
...
template <class T>
GAAlleleSetArray<T>& GAAlleleSetArray<T>::operator=(const GAAlleleSetArray<T>& orig)
{
    if (this == &orig)
        return *this;
    unsigned int i;
...
template <class T>
int GAAlleleSetArray<T>::add(const GAAlleleSet<T>& s)
{
    if (sz + 1 > SZ) {
        SZ += csz;
...
template <class T>
int GAAlleleSetArray<T>::add(unsigned int n, const T a[])
{
    if (sz + 1 > SZ) {
        SZ += csz;
        GAAlleleSet<T>** tmp = aset;
...
template <class T>
int GAAlleleSetArray<T>::add(const T& lower, const T& upper, GAAllele::BoundType lb, GAAllele::BoundType ub)
{
    if (sz + 1 > SZ) {
        SZ += csz;
        GAAlleleSet<T>** tmp = aset;
...
template <class T>
int GAAlleleSetArray<T>::add(const T& lower, const T& upper, const T& increment, GAAllele::BoundType lb,
                             GAAllele::BoundType ub)
{
    if (sz + 1 > SZ) {
        SZ += csz;
...
template <class T>
int GAAlleleSetArray<T>::remove(unsigned int n)
{
    if (n >= sz)
        return 1;
    delete aset[n];
