...
class GAList : public GAListBASE
{
  public:
    ...
    T *warp(const GAListIter<T> &i) { return ((i.list == this) ? iter.warp(i) : (T *)0); }
...
class GAListIter : public GAListIterBASE
{
  public:
    ...
    T *head()
    {
        return (((node = GAListIterBASE::head()) != (GANodeBASE *)0) ? &((GANode<T> *)GAListIterBASE::head())->contents
                                                                     : (T *)0);
    }
    T *tail()
    {
        return (((node = GAListIterBASE::tail()) != (GANodeBASE *)0) ? &((GANode<T> *)GAListIterBASE::tail())->contents
                                                                     : (T *)0);
    }
...
extern GANodeBASE *_GAListTraverse(unsigned int index, unsigned int &cur, GANodeBASE *node);
...
void GAList<T>::copy(const GAList<T> &orig)
{
    while (hd)
        delete GAListBASE::remove(DYN_CAST(GANode<T> *, hd));
    hd = _GAListCopy(DYN_CAST(GANode<T> *, orig.hd), DYN_CAST(GANode<T> *, orig.hd));
    iter.node = hd;
    sz = orig.sz;
    csz = orig.csz;
}
...
// list.
template <class T>
GAList<T> *GAList<T>::clone(unsigned int i) const
{
...
template <class T>
int GAList<T>::swap(unsigned int a, unsigned int b)
{
...
template <class T>
GANode<T> *_GAListCopy(GANode<T> *node, GANode<T> *head)
{
    if (!node)
        return (GANode<T> *)0;
