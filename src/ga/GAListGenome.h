...
template <class T>
int GAListIsHole(const GAListGenome<T> &, const GAListGenome<T> &, int, int, int);

template <class T>
const char *GAListGenome<T>::className() const
{
    return "GAListGenome";
}
template <class T>
int GAListGenome<T>::classID() const
{
...
GAListGenome<T>::GAListGenome(GAGenome::Evaluator f, void *u)
        : GAList<T>(), GAGenome(DEFAULT_LIST_INITIALIZER, DEFAULT_LIST_MUTATOR, DEFAULT_LIST_COMPARATOR)
{
    evaluator(f);
    userData(u);
...
template <class T>
void GAListGenome<T>::copy(const GAGenome &orig)
{
    if (&orig == this)
        return;
...
template <class T>
int GAListGenome<T>::write(STD_OSTREAM &os) const
{
...
template <class T>
int GAListGenome<T>::equal(const GAGenome &c) const
{
    if (this == &c)
        return 1;
...
//   After the mutation the iterator is left at the head of the list.
template <class T>
int GAListGenome<T>::SwapMutator(GAGenome &c, float pmut)
{
    GAListGenome<T> &child = DYN_CAST(GAListGenome<T> &, c);
    register int n, i;
    if (pmut <= 0.0)
...
template <class T>
int GAListGenome<T>::OnePointCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
...
template <class T>
int GAListGenome<T>::PartialMatchCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GAListGenome<T> &mom = DYN_CAST(const GAListGenome<T> &, p1);
    const GAListGenome<T> &dad = DYN_CAST(const GAListGenome<T> &, p2);
...
template <class T>
int GAListIsHole(const GAListGenome<T> &child, const GAListGenome<T> &parent, int index, int a, int b)
{
...
template <class T>
int GAListGenome<T>::OrderCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GAListGenome<T> &mom = DYN_CAST(const GAListGenome<T> &, p1);
    const GAListGenome<T> &dad = DYN_CAST(const GAListGenome<T> &, p2);
    ...
    if (c1) {
        ...
        for (i = a, sis.warp(a), diter.warp(a); i < b; i++, sis.next(), diter.next()) {
    ...
    if (c2) {
        ...
        // Now put the 'holes' in the proper order within the crossover section.
        for (i = a, bro.warp(a), miter.warp(a); i < b; i++, bro.next(), miter.next()) {
...
template <class T>
int GAListGenome<T>::CycleCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
    const GAListGenome<T> &mom = DYN_CAST(const GAListGenome<T> &, p1);
    const GAListGenome<T> &dad = DYN_CAST(const GAListGenome<T> &, p2);
