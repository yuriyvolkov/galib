...
class GASigmaTruncationScaling : public GAScalingScheme
{
  public:
    ...
    GASigmaTruncationScaling(float m = gaDefSigmaTruncationMultiplier) { multiplier(m); }
    ...
    virtual GAScalingScheme* clone() const { return new GASigmaTruncationScaling(*this); }
    virtual void evaluate(const GAPopulation& p);
    virtual void copy(const GAScalingScheme& arg)
    {
        if (&arg != this) {
...
class GAPowerLawScaling : public GAScalingScheme
{
  public:
    ...
    virtual GAScalingScheme* clone() const { return new GAPowerLawScaling(*this); }
    virtual void evaluate(const GAPopulation& p);
    virtual void copy(const GAScalingScheme& arg)
    {
        if (&arg != this) {
...
class GASharing : public GAScalingScheme
{
  public:
    ...
    GASharing(GAGenome::Comparator func, float cut = gaDefSharingCutoff, float a = 1.0)
    {
        N = 0;
        d = (float*)0;
        df = func;
