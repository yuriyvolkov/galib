...
int GAParameterList::add(const char* fn, const char* sn, GAParameter::Type t, const void* v)
{
    int status = -1;
    if (n == N) {
        N += PRM_CHUNKSIZE;
    ...
    int found = 0;
    for (unsigned int i = 0; i < n && !found; i++) {
        if (strcmp(fn, p[i]->fullname()) == 0 && strcmp(sn, p[i]->shrtname()) == 0)
            found = 1;
    }
    if (!found) {
        cur = n;
...
int GAParameterList::write(STD_OSTREAM& os) const
{
    for (unsigned int i = 0; i < n; i++) {
        int ival;
        float fval;
...
int GAParameterList::read(STD_ISTREAM& is, GABoolean flag)
{
    int nfound = 0;
    if (n == 0)
        return nfound;
...
int GAParameterList::read(const char* filename, GABoolean flag)
{
...
int GAParameterList::parse(int& argc, char* argv[], GABoolean flag)
{
    int nfound = 0;
    if (n == 0)
        return nfound;
    ...
    for (int i = 0; i < argc; i++) {
        ...
        for (unsigned int j = 0; j < n && found == 0; j++) {
            if (strcmp(p[j]->shrtname(), argv[i]) == 0 || strcmp(p[j]->fullname(), argv[i]) == 0) {
                found = 1;
                ...
                if (++i >= argc) {
                    ...
                    case GAParameter::BOOLEAN:
                        if (IsNumeric(argv[i])) {
                            ...
                            if (strcmp(argv[i], "true") == 0 || strcmp(argv[i], "True") == 0 ||
                                strcmp(argv[i], "TRUE") == 0 || strcmp(argv[i], "t") == 0 || strcmp(argv[i], "T") == 0)
                                ival = 1;
                        }
