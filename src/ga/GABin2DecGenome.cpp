...
GABin2DecPhenotypeCore::GABin2DecPhenotypeCore() : csz(GA_B2D_CHUNKSIZE), n(0), N(0), sz(0)
{
    nbits = oset = 0;
    minval = maxval = 0;
    cnt = 1;
...
GABin2DecPhenotypeCore::GABin2DecPhenotypeCore(const GABin2DecPhenotypeCore& p) : csz(p.csz), n(p.n), N(p.N), sz(p.sz)
{
...
GABin2DecPhenotypeCore& GABin2DecPhenotypeCore::operator=(const GABin2DecPhenotypeCore& p)
{
    if (&p == this)
        return *this;
...
void GABin2DecPhenotype::remove(unsigned int x)
{
    if (x >= core->n)
        return;
    memmove(&(core->nbits[x]), &(core->nbits[x + 1]), (core->n - x - 1) * sizeof(unsigned short));
    memmove(&(core->oset[x]), &(core->oset[x + 1]), (core->n - x - 1) * sizeof(unsigned short));
    memmove(&(core->minval[x]), &(core->minval[x + 1]), (core->n - x - 1) * sizeof(float));
    memmove(&(core->maxval[x]), &(core->maxval[x + 1]), (core->n - x - 1) * sizeof(float));
    core->n -= 1;
}
...
const GABin2DecPhenotype& GABin2DecGenome::phenotypes(const GABin2DecPhenotype& p)
{
    *ptype = p;
...
float GABin2DecGenome::phenotype(unsigned int n) const
{
    ...
    float val = 0.0;
    decode(val, &(data[ptype->offset(n)]), ptype->length(n), ptype->min(n), ptype->max(n));
    return val;
}
...
float GABin2DecGenome::phenotype(unsigned int n, float val)
{
    ...
    encode(val, &(data[ptype->offset(n)]), ptype->length(n), ptype->min(n), ptype->max(n));
    return val;
}
...
int GABin2DecGenome::read(STD_ISTREAM& is)
{
    float value;
...
int GABin2DecGenome::write(STD_OSTREAM& os) const
{
    for (unsigned int i = 0; i < phenotypes().nPhenotypes(); i++)
        os << phenotype(i) << " ";
    return 0;
...
int GABin2DecGenome::equal(const GAGenome& g) const
{
    ...
    return ((GA1DBinaryStringGenome::equal(b) && *ptype == *(b.ptype)) ? gaTrue : gaFalse);
}

int GABin2DecGenome::notequal(const GAGenome& g) const
{
    GABin2DecGenome& b = (GABin2DecGenome&)g;
    return ((GA1DBinaryStringGenome::notequal(b) || *ptype != *(b.ptype)) ? gaTrue : gaFalse);
}
