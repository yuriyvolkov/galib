...
void GAPopulation::copy(const GAPopulation &arg)
{
    ...
    scaled = gaFalse;
    if (arg.scaled == gaTrue)
        scale();

    slct = arg.slct->clone();
    slct->assign(*this);
    selectready = gaFalse;
    if (arg.selectready == gaTrue)
        prepselect();
...
void GAPopulation::sort(GABoolean flag, SortBasis basis) const
{
...
void GAPopulation::statistics(GABoolean flag) const
{
...
// a const population.
void GAPopulation::scale(GABoolean flag) const
{
...
void GAPopulation::diversity(GABoolean flag) const
{
    if (divved == gaTrue && flag != gaTrue)
        return;
    GAPopulation *This = (GAPopulation *)this;
    ...
    if (n > 1) {
        ...
        for (unsigned int i = 0; i < n; i++) {
            ...
            for (unsigned int j = i + 1; j < n; j++) {
                This->indDiv[j * n + i] = This->indDiv[i * n + j] = individual(i).compare(individual(j));
...
void GAPopulation::prepselect(GABoolean flag) const
{
    if (selectready == gaTrue && flag != gaTrue)
        return;
    GAPopulation *This = (GAPopulation *)this;
    This->slct->update();
...
GAGenome *GAPopulation::add(const GAGenome &g) { return GAPopulation::add(g.clone()); }
...
void GAPopulation::write(STD_OSTREAM &os, SortBasis basis) const
{
    for (unsigned int i = 0; i < n; i++) {
        if (basis == RAW)
            os << *rind[i] << "\n";
