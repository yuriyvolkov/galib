...
template <class T>
int GA1DArrayGenome<T>::resizeBehaviour(unsigned int lower,
                                        unsigned int upper) {
  if (upper < lower) {
...
GA1DArrayAlleleGenome<T>::GA1DArrayAlleleGenome(unsigned int length,
  ...
  GAGenome::crossover(
      GA1DArrayAlleleGenome<T>::DEFAULT_1DARRAY_ALLELE_CROSSOVER);
}
...
int GA1DArrayGenome<T>::PartialMatchCrossover(const GAGenome &p1,
  ...
  if (c1 && c2) {
    ...
    for (i = a, index = a; i < b; i++, index++) {
      for (j = 0; j < sis.length() - 1 && sis.gene(j) != parent2->gene(index);
           j++)
        ;
      sis.swap(i, j);
    }
