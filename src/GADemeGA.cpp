...
int GADemeGA::setptr(const char* name, const void* value)
{
    ...
    if (strcmp(name, gaNnPopulations) == 0 || strcmp(name, gaSNnPopulations) == 0) {
        nPopulations(*((int*)value));
        status = 0;
    } else if (strcmp(name, gaNnMigration) == 0 || strcmp(name, gaSNnMigration) == 0) {
...
void GADemeGA::step()
{
    ...
    for (unsigned int ii = 0; ii < npop; ii++) {
        for (i = 0; i < nrepl[ii] - 1; i += 2) { // takes care of odd population
            ...
            c1 = c2 = 0;
            if (GAFlipCoin(pc)) {
                pstats[ii].numcro += (*scross)(*mom, *dad, &tmppop->individual(i), &tmppop->individual(i + 1));
                c1 = c2 = 1;
            } else {
        ...
        if (nrepl[ii] % 2 != 0) { // do the remaining population member
            ...
            c1 = 0;
            if (GAFlipCoin(pc)) {
                pstats[ii].numcro += (*scross)(*mom, *dad, &tmppop->individual(i), (GAGenome*)0);
                c1 = 1;
            } else {
                if (GARandomBit())
                    tmppop->individual(i).copy(*mom);
        ...
        deme[ii]->scale();
        for (i = 0; i < nrepl[ii]; i++)
            tmppop->replace(deme[ii]->remove(GAPopulation::WORST, GAPopulation::SCALED), i);
