...
template <class T>
const char *GATreeGenome<T>::className() const
{
    return "GATreeGenome";
}
template <class T>
int GATreeGenome<T>::classID() const
{
...
GATreeGenome<T>::GATreeGenome(GAGenome::Evaluator f, void *u)
    : GATree<T>(), GAGenome(DEFAULT_TREE_INITIALIZER, DEFAULT_TREE_MUTATOR, DEFAULT_TREE_COMPARATOR)
{
    evaluator(f);
    userData(u);
...
template <class T>
void GATreeGenome<T>::copy(const GAGenome &orig)
{
    if (&orig == this)
        return;
...
template <class T>
void _tt(STD_OSTREAM &os, GANode<T> *n)
{
    if (!n)
        return;
...
template <class T>
int GATreeGenome<T>::write(STD_OSTREAM &os) const
{
...
template <class T>
int GATreeGenome<T>::equal(const GAGenome &c) const
{
    if (this == &c)
        return 1;
...
//   After the mutation the iterator is left at the root of the tree.
template <class T>
int GATreeGenome<T>::SwapNodeMutator(GAGenome &c, float pmut)
{
    GATreeGenome<T> &child = DYN_CAST(GATreeGenome<T> &, c);
    register int n, i;
    if (pmut <= 0.0)
...
template <class T>
int GATreeGenome<T>::OnePointCrossover(const GAGenome &p1, const GAGenome &p2, GAGenome *c1, GAGenome *c2)
{
