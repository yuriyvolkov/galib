...
RobotPathGenome::RobotPathGenome(int nrobots, int pathlength) : GAGenome(Initializer, Mutator, Comparator)
{
    evaluator(Evaluator);
    crossover(Crossover);
    n = nrobots;
...
GAGenome* RobotPathGenome::clone(GAGenome::CloneMethod) const { return new RobotPathGenome(*this); }

int RobotPathGenome::equal(const GAGenome& g) const
{
    RobotPathGenome& genome = (RobotPathGenome&)g;
    int flag = 0;
...
int RobotPathGenome::read(istream& is)
{
    for (int i = 0; i < n; i++)
        is >> *(list[i]);
    return is.fail() ? 1 : 0;
...
int RobotPathGenome::write(ostream& os) const
{
    for (int i = 0; i < n; i++)
        os << "list " << i << ":\t" << *(list[i]) << "\n";
    return os.fail() ? 1 : 0;
...
// crossover object).
int RobotPathGenome::Crossover(const GAGenome& a, const GAGenome& b, GAGenome* c, GAGenome* d)
{
    ...
    if (c && d) {
        ...
        for (int i = 0; i < mom.npaths(); i++)
            GAListGenome<int>::PartialMatchCrossover(mom.path(i), dad.path(i), &sis.path(i), &bro.path(i));
        ...
        RobotPathGenome& sis = (RobotPathGenome&)*c;
        for (int i = 0; i < mom.npaths(); i++)
            GAListGenome<int>::PartialMatchCrossover(mom.path(i), dad.path(i), &sis.path(i), 0);
        sis._evaluated = gaFalse;
        n = 1;
    } else if (d) {
        RobotPathGenome& bro = (RobotPathGenome&)*d;
        for (int i = 0; i < mom.npaths(); i++)
            GAListGenome<int>::PartialMatchCrossover(mom.path(i), dad.path(i), 0, &bro.path(i));
        bro._evaluated = gaFalse;
        n = 1;
    }
    return n;
...
template <>
int GAListGenome<int>::write(ostream& os) const
{
    int* cur, *head;
