...
float Objective2(GAGenome& g)
{
    ...
    float y = 0.5 + 0.6 * sin(M_PI * v) + 0.2 * sin(3 * M_PI * v) + 0.1 * sin(5 * M_PI * v) + 0.02 * sin(7 * M_PI * v) +
              0.01 * sin(7 * M_PI * v);
    if (v < -0.23 || v > 1.23)
        y = 0;
    if (y < 0)
...
int Mutator(GAGenome& g, float pmut)
{
    ...
    int nmut = 0;
    if (GAFlipCoin(pmut)) {
        genome.gene(0, genome.gene(0) + GARandomFloat() * (GARandomFloat() - GARandomFloat()));
        nmut = 1;
    }
    return nmut;
}
