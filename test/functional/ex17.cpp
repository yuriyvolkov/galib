...
float objective(GAGenome &c)
{
    ...
    value += 10 * (genome.gene(genome.width() - 1, 0) == genome.alleleset().allele(0));
    value += 10 * (genome.gene(0, genome.height() - 1) == genome.alleleset().allele(0));
    value += 10 * (genome.gene(genome.width() - 1, genome.height() - 1) == genome.alleleset().allele(0));
    return (value);
}
...
// each other up.
template <>
int GA2DArrayAlleleGenome<int>::write(ostream &os) const
{
