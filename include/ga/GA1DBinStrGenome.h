...
class GA1DBinaryStringGenome : public GABinaryString, public GAGenome
{
  ...
  public:
    GA1DBinaryStringGenome(unsigned int x, GAGenome::Evaluator f = (GAGenome::Evaluator)0, void* u = (void*)0);
    ...
    int equal(const GA1DBinaryStringGenome& orig, unsigned int r, unsigned int x, unsigned int length) const;
...
inline void GA1DBinaryStringGenome::copy(const GA1DBinaryStringGenome& orig, unsigned int r, unsigned int x,
                                         unsigned int l)
{
...
inline void GA1DBinaryStringGenome::move(unsigned int x, unsigned int srcx, unsigned int l)
{
    if (srcx + l > nx)
        l = nx - srcx;
    if (x + l > nx)
